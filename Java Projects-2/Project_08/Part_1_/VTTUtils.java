...
public class VTTUtils
{
  ...
  public static Font getFont( String strName, int nStyle, int nSize )
  {
    return new Font( validateFontName( strName ),
                     FONT_STYLE[( nStyle > -1 && nStyle < 3 ) ? nStyle : 0],
                     ( nSize >= 6 && nSize <= 144 ) ? nSize : 10 );
  }
  ...
  public static String validateFontName( String strName )
  {
    ...
    {
      if ( strName.equals( strFontNames[nLup] ) )
      {
        bGoodFont = true;
        nLup      = strFontNames.length;
      }
    }

    if ( !bGoodFont )
    {
      strHoldName = new String( "Arial" );
    }
